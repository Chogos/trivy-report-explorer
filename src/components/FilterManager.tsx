...
const FilterManager: React.FC = () => {
    ...
    switch (filter.type) {
      ...
      case 'resourceType':
        const resourceOptions =
          resourceTypes.length > 0 ? resourceTypes : filter.options || [];
  ...
  const alwaysVisibleFilters = availableFilters.filter(
    f => f.type === 'severity' || f.isReportSpecific
  );
  const expandableFilters = availableFilters.filter(
    f => f.type !== 'severity' && !f.isReportSpecific
  );
                  ...
                  return value.map((val: string) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleMultiSelectChange(filter.id, val)
                          }
