...
const Filters: React.FC = () => {
  const { report, reportType, filters, setFilters, sortOptions, setSortOptions, getFilterableFields } = useReport();
    ...
    <div className="bg-white rounded-md border border-github-border p-4 mb-6 shadow-sm">
        ...
        <div className="mb-4 w-full">
          ...
          <div className="flex flex-wrap gap-2">
            {severityOptions.map((option) => (
              ...
              >
                <span className={`w-2 h-2 rounded-full ${option.color} mr-2 ${!filters.severity.includes(option.value) && 'opacity-70'}`}></span>
                {option.label}
              </button>
            ))}
          </div>
      ...
      {isExpanded && (
        <div className="pt-4 border-t border-github-border grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="max-h-32 overflow-y-auto pr-2 space-y-1">
                {resourceTypes.map((option) => (
                  <div key={option.value} className="flex items-center">
                    ...
                    />
                    <label htmlFor={`resource-${option.value}`} className="ml-2 text-sm text-gray-700 cursor-pointer">
                      {option.label}
                    </label>
                  </div>
                ))}
          ...
          {(reportType === ReportType.TRIVY_VULNERABILITY || reportType === ReportType.TRIVY_LICENSE) && (
            <div>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {filters.severity.map(severity => {
                ...
                return (
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full ${option?.color} mr-1`}></span>
                    {option?.label}
                    <button
